...
AdventureMapFactory.createAdventureMap = function(level) {
    ...
    */
    ...
    switch(level) {
        case 1:
            // forest level
            mapOptions[MapOptions.TERRAIN_FILL] = Math.min(.2 + 2*difficulty/10, 1);
            mapOptions[MapOptions.TERRAIN_DENSITY] = Math.min(.2 + 2*difficulty/10, 1);
        ...
        case 2:
            // camp level
            mapOptions[MapOptions.TERRAIN_FILL] = Math.max(1 - 2*difficulty/10, .4);
