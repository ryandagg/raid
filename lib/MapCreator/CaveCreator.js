...
function cave(height, width, mapOptions) {
    ...
    // Choose a few centers for large openings in the cave.
    ...
    // Grow large rooms randomly around centers
    ...
    for (i = 2; i < centers.length; i++) {
        var minX = Math.round(Math.random()) + 2;
        var maxX = Math.round(Math.random()) + 2;
        var minY = Math.round(Math.random()) + 2;
        var maxY = Math.round(Math.random()) + 2;
        ...
        // Occasionally add som
        // e pools of water
        if (Math.random() < .25) {
            var poolCenter = centers[i].add(Direction.randomDirection());
            var poolSize = MapUtils.getRandomFromRange(0,1);
            var poolBoundLeft = new MapLocation(poolCenter.x + 1, poolCenter.y - poolSize);
            var poolBoundRight = new MapLocation(poolCenter.x - poolSize, poolCenter.y + 1);
            map.createTiles(poolBoundLeft, poolBoundRight, TerrainType.WATER);
        }
